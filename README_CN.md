...
适用于纯净的 Linux 服务器，自动安装 k3s、ingress‑nginx、Helm 并部署 MCPCAN 平台。
...
成功后，访问 `http://<你的公网IP>` 即可使用。

**自定义安装（Helm）**

适用于自定义域名、HTTPS 或修改默认配置的场景。

```bash
# 1. 安装依赖（若已有 k3s/Helm 可跳过）
./scripts/install-run-environment.sh       # 国际源
# ./scripts/install-run-environment.sh --cn  # 中国源

# 2. 复制并修改配置
cp helm/values.yaml helm/values-custom.yaml
# 编辑 helm/values-custom.yaml 设置 global.domain 等参数
