...
适用于 Kubernetes 环境部署，请参考 [Helm 部署指南](deploy/helm/)。
...
| **MCP-Gateway** | `backend/cmd/gateway/`     | 统一网关转发、安全认证与 SSE 流处理      |
| **MCP-Authz**   | `backend/cmd/authz/`       | 权限系统、RBAC、用户与部门管理           |
| **MCP-Sidecar** | `backend/cmd/mcp-sidecar/` | MCP 服务运行时的流量代理与协议转换       |
| **MCP-Init**    | `backend/cmd/init/`        | 系统初始化与基础数据加载服务             |
